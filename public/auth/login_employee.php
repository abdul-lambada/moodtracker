...
$redirect = session_get('__auth_redirect', '../mood/create.php');
...
<head>
    ...
    <title>Masuk Karyawan | MoodTracker</title>
...
<body class="bg-mood-soft text-mood-ink min-h-screen flex items-center justify-center">
    <div class="w-full max-w-lg px-6 py-10 bg-mood-surface border border-mood-border rounded-3xl shadow-xl page-enter">
        <div class="text-center space-y-2 page-enter-delay">
            ...
            <h1 class="text-2xl font-semibold text-mood-ink">Masuk Karyawan</h1>
            <p class="text-sm text-mood-muted">Gunakan nomor bundy dan kata sandi Anda untuk mencatat mood.</p>
        </div>
        ...
        <form action="login_employee_process.php" method="POST" class="mt-8 space-y-5 page-enter-delay-lg">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="redirect" value="<?php echo htmlspecialchars($redirect); ?>">
            <div>
                <label for="no_bundy" class="block text-sm font-semibold text-mood-muted">Nomor Bundy</label>
                <input type="text" id="no_bundy" name="no_bundy" class="mt-2 w-full rounded-2xl border border-mood-border px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-mood-primary" placeholder="Masukkan nomor bundy" required>
            </div>
            <div>
                <label for="password" class="block text-sm font-semibold text-mood-muted">Kata Sandi</label>
                <input type="password" id="password" name="password" class="mt-2 w-full rounded-2xl border border-mood-border px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-mood-primary" placeholder="Masukkan kata sandi" required>
            </div>
            <button type="submit" class="w-full bg-mood-primary hover:bg-mood-primary-dark text-white font-semibold py-3 rounded-2xl">Masuk</button>
        </form>

        <div class="mt-6 text-center text-sm text-mood-muted space-y-2">
            <div>
                Belum punya akun? <a href="register.php" class="text-slate-600 hover:text-slate-500">Daftar karyawan</a>
            </div>
